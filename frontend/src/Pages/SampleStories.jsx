...
// Sample featured stories data - in a real app, this would come from your API
...
const SampleStories = () => {
  ...
  const handleGenerateStory = async (e) => {
    ...
    if (!storyContext.trim()) {
      alert('Please provide some context for the story.');
      return;
    }
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      if (err.name !== 'AbortError') {
        alert('Could not share the story. Please try again or copy it manually.');
      }
    }
  };
  ...
  // const currentStory = featuredStories[showFullStory !== null ? showFullStory : currentStoryIndex];
          ...
          <div className="relative
            ...
            >
              {featuredStories.map((story, index) => (
                <div 
                  key={story.id}
                  className={`flex-shrink-0 w-10/12 sm:w-8/12 md:w-7/12 lg:w-5/12 px-4 transition-all duration-300 ${
